...
> users yet.
>
> Today it requires the guarded `stronkpi` launcher, role manifests, Model
> Context Protocol (MCP) policy, trusted runtime pins, and config generated by
> the private maintainer workstation setup. Those pieces are being extracted
> into the public `stronk-pi-setup` flow over time.
